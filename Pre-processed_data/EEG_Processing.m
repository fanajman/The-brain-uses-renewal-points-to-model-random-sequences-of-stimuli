...
function EEG_Processing(base_rootdir)


% directory to load the raw data
%base_rootdir = 'Z:\servidor\MATLAB\EEGretrieving_raw\raw_data';


% directory to save the preprocessed data
%base_destdir = 'Z:\servidor\MATLAB\EEGretrieving-master\OrigData';
%base_destdir = '/Users/pesquisador/Desktop/servidor/MATLAB/ExportFolder/Pre-processed_data';
...
for v = [1:3  5:length(files) ]	% we skip volunteer V04 as indicate in the article 
	...
	EEG = pop_readegi( fullfile( rootdir, files(v).name ), [], [], 'GSN129.sfp' );
	...
	%1 - Fp2, 2 - Fz, 3 - Fp1, 4 - F3, 5 -  F7, 6 - C3, 7 - T7, 8 - P3, 9 - P7, 10 - Pz, 11 - O1, 12 - O2, 13 - P4, 14 - P8, 15 - C4, 16 - T8, 17 - F8, 18 - F4
    %set_10_20 = {'E9','E11','E22','E24','E33','E36','E45','E52','E58','E62','E70','E83','E92', 'E96','E104','E108','E122','E124'};         
    %EEG = pop_select( EEG, 'channel', set_10_20 ); 
	%set_10_20 = {'E9','E11','E22','E24','E33','E36','E45','E52','E58','E62','E70','E83','E92', 'E96','E104','E108','E122','E124'};         
    %EEG = pop_select( EEG, 'channel', set_10_20 ); 
	evnts = cell(1, length(EEG.chanlocs));
	for inds = 1:length(EEG.chanlocs)
		evnts{inds} = EEG.chanlocs(inds).labels;
	end
	% sharing in blocks
    ...
    filename = [fullfile(EEG.setname(1:3)) 'NoFilter']; 
    save(filename, 'data', 'evnts')
end